...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::ValuesIn(std::vector<ComputeBooleanUnionTestParams>{
            ...
            // Tangent disks, see elements_intersections_test.cpp's two
            // circle/circle tangency cases.
            ComputeBooleanUnionTestParams::read_json(
                    (fs::path("data") / "tests" / "boolean_operations" / "union" / "038.json").string()),
            ComputeBooleanUnionTestParams::read_json(
                    (fs::path("data") / "tests" / "boolean_operations" / "union" / "039.json").string()),
            ComputeBooleanUnionTestParams::read_json(
                    (fs::path("data") / "tests" / "boolean_operations" / "union" / "040.json").string()),
        }),
